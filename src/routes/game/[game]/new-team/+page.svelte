...
<div class="flex flex-wrap">
	<div class="w-screen flex justify-center">
		<div class="grid grid-cols-6 gap-2 w-full max-w-5xl">
			{#each Array(6) as _, i}
				<div class="aspect-square w-full card shadow-sm flex items-center justify-center bg-red-200">
					{#if selected[i] !== undefined}
						<img src={pokemon[selected[i]].sprite} alt={pokemon[selected[i]].name} class="max-h-full max-w-full object-contain"/>
					{/if}
				</div>
			{/each}
		</div>
	...
	<div class="w-1/2 mx-auto flex flex-col items-center">
		...
		<div class="join sticky bottom-0 bg-base-100 w-full flex justify-center py-2">
		...
		{#if selectedPokemon}
			<div>
				...
				<div class="card w-full shadow-sm">
					...
					<div class="card-body">
						<h2 class="card-title justify-center">{toTitleCase(selectedPokemon.name)}</h2>

						<div class="justify-center"> Typing: 
						...
						<p>
							{#each selectedPokemon.stats as stat}
								<div>{toTitleCase(stat.name)} : {stat.stat}</div>
							{/each}
						</p>
						...
						<div class="justify-center">
							...
							{#if selectedPokemon.locations.find(lo => lo.versions.includes(game))}
								{toTitleCase(selectedPokemon.locations.find(lo => lo.versions.includes(game)).location_area)}
							{:else}
								Evolve
							{/if}
						</div>
